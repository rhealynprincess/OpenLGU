...
class User extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            array('email, password, oldPassword, newPassword, confirmPassword,
                  first_name, middle_name, last_name', 'length', 'max'=>32),
            //array('newPassword, confirmPassword', 'length', 'min'=>6),
            array('email, first_name, middle_name, last_name', 'required'),
            array('newPassword, confirmPassword', 'required', 'on'=>'insert'),
            array('email, newEmail', 'email'),
            array('email, newEmail', 'unique', 'attributeName'=>'email', 'message'=>'Email has already been taken.'),
            array('confirmPassword', 'compare', 'compareAttribute'=>'newPassword',
                  'on'=>'insert, changePassword', 'message'=>'Passwords must match.'),
            array('user_id, email, first_name, middle_name, last_name, role, status, create_date', 'safe'),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('user_id, email, first_name, middle_name, last_name, role,
